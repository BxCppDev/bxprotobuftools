...
#ifndef BXPROTOBUFTOOLS_STD_LIST_CONVERTERS_H
#define BXPROTOBUFTOOLS_STD_LIST_CONVERTERS_H
...
namespace protobuftools {
  ...
  public:
    ...
    static void protobufize(::protobuftools::message_node & node_, std::list<T> & x_)
    {
      ...
      for (typename std::list<T>::iterator iter = x_.begin(); iter != x_.end(); iter++) {
        T & xelem = *iter;
    ...
    static void deprotobufize(message_node & node_, std::list<T> & x_)
    {
      BX_PROTOBUF_MESSAGE_THROW_IF(! node_.is_field(), message_exception,
                                   node_.get_message(), "Missing field descriptor!");
      const google::protobuf::FieldDescriptor * field_descriptor = node_.get_field_descriptor();
...
#endif // BXPROTOBUFTOOLS_STD_LIST_CONVERTERS_H
